...
# 9. 수행결과(테스트/시연 페이지)
# 10. 한 줄 회고
💰 서예찬: 모델의 성능을 높이는데 집중을하다 시간분배를 잘못해 전체적인 퍼포먼스부분에서 기준점을 만족하지못한것같아 많은점에서 아쉽게느껴졌던 프로젝트였던것같습니다.
💰 전성원: 많은 변수들이 있어서 다사다난했던 프로젝트였던 것 같습니다, 솔직히 파인튜닝을 진행하면서 더 성능 좋은 모델을 뽑아보고싶었는데 장비나 비용 등의 외적인 원인으로 선택과 타협으로 퍼포먼스적인 부분을 챙길 수 없었던 점이 안타까웠습니다.  
💰 조민훈: 클라이언트의 니즈와 실사용자의 니즈를 구체적으로 파악해야하는 것이 매우 중요하다.

💰 최재동: 이번에 한 프로젝트는 django, aws ec2, langchain등 다양한 기능을 한데 엮는 프로젝트라 여러가지 어려움이 많았습니다. 각각의 요소들을 연결하면서 초기 설계를 정확히 하는 것의 중요성을 느꼈습니다.
